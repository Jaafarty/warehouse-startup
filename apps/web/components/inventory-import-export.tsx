...
export function InventoryImportExport({ storeId, categories, products }: Props) {
  ...
  async function handleImport() {
    ...
    const raw: RawRow[] = parsed.map((row) => {
      ...
      return {
        ...
        lowStockThreshold:
          lst === undefined ? undefined : Math.max(0, Math.floor(lst)),
      };
    });
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <DialogTrigger className="inline-flex items-center justify-center rounded-lg border border-input bg-transparent px-3 h-8 text-sm font-medium hover:bg-muted gap-1.5">
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload an .xlsx or .csv file. Dates must be YYYY-MM-DD or DD/MM/YYYY.
            </DialogDescription>
          </DialogHeader>
            ...
            {parsed.length > 0 && (
              <div className="rounded-lg border p-3 bg-muted/20 text-sm space-y-1">
                ...
                <p className="text-muted-foreground text-xs">
                  ...
                  barcode, or name) will have their fields updated and any quantity
                  added as restock.
                </p>
              </div>
            )}
